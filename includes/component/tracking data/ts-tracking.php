...
class Bkap_TS_tracking {
	...
	public function __construct( $ts_plugin_prefix = '', $ts_plugin_name = '', $ts_blog_post_link = '', $ts_plugin_context = '', $ts_plugin_url = '', $setting_page = '', $add_setting = '', $setting_section = '', $setting_register = '' ) {
		...
		// Tracking Data
		add_action( 'admin_notices', array( 'Bkap_TS_tracking', 'ts_track_usage_data' ) );
		add_action( 'admin_footer', array( 'Bkap_TS_tracking', 'ts_admin_notices_scripts' ) );
		add_action( 'wp_ajax_' . self::$plugin_prefix . '_admin_notices', array( 'Bkap_TS_tracking', 'ts_admin_notices' ) );

		add_filter( 'cron_schedules', array( 'Bkap_TS_tracking', 'ts_add_cron_schedule' ) );

		add_action( self::$plugin_prefix . '_add_new_settings', array( 'Bkap_TS_tracking', 'ts_add_reset_tracking_setting' ) );

		add_action( 'admin_init', array( 'Bkap_TS_tracking', 'ts_reset_tracking_setting' ) );
	...
	public static function ts_add_reset_tracking_setting() {
		...
		add_settings_field(
			...
			array( 'Bkap_TS_tracking', 'ts_rereset_tracking_callback' ),
	...
	public static function ts_admin_notices() {
		...
		Bkap_TS_Tracker::ts_send_tracking_data( false );
		die();
	}
	...
	private static function ts_tracking_actions() {
		if ( isset( $_GET[ self::$plugin_prefix . '_tracker_optin' ] ) && isset( $_GET[ self::$plugin_prefix . '_tracker_nonce' ] ) && wp_verify_nonce( $_GET[ self::$plugin_prefix . '_tracker_nonce' ], self::$plugin_prefix . '_tracker_optin' ) ) {
			...
			Bkap_TS_Tracker::ts_send_tracking_data( true );
