 ...
 * @package  BKAP/Emails
 * @class    BKAP_Email_Booking_Cancelled
 * @category Classes
 * @extends  WC_Email
 * @since    2.5
 */
...
class BKAP_Email_Booking_Cancelled extends WC_Email {
	...
	function __construct() {
		...
		$this->recipient = apply_filters( 'bkap_booking_cancelled_additional_email', $this->get_option( 'recipient', '' ) );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'recipient'          => array(
				'title'       => __( 'Recipient(s)', 'woocommerce-booking' ),
				'type'        => 'text',
				'description' => sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to <code>%s</code>.', 'woocommerce-booking' ), esc_attr( get_option( 'admin_email' ) ) ),
				'default'     => '',
			),
			'heading'            => array(
