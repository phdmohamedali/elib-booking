...
$order = ! empty( $booking ) ? wc_get_order( $booking->order_id ) : array();

$product_title = ! empty( $booking ) ? $booking->product_title : 'Car';
$booking_date  = ! empty( $booking ) ? $booking->item_booking_date : date( 'j-n-Y' );
...
$html .= '</tr>';
$html .= '<tr>';
$html .= '<th style="text-align:left; border: 1px solid #eee;" scope="row">' . __( ! empty( $booking ) ? $booking->start_date_label : 'Start Date', 'woocommerce-booking' ) . '</th>';
...
if ( $order && ! empty( $booking ) && $booking->customer_id > 0 ) {
	$html .= '<tr>';
